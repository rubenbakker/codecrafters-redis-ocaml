...
type id_t = { millis : int64; sequence : int } [@@deriving compare, equal, sexp]
...
let min_id = { millis = 0L; sequence = 0 }
let max_id = { millis = Int64.max_value; sequence = Int.max_value }
let id_to_string id = Stdlib.Printf.sprintf "%Ld-%d" id.millis id.sequence
let empty () = []
...
let parse_entry_id (id : string) (last_entry_id : id_t) :
  ...
  let new_sequence millis =
    if Int64.(last_entry_id.millis = millis) then last_entry_id.sequence + 1
    else 0
  in
  ...
  | [ millis; "*" ] ->
      Ok
        {
          millis = Int64.of_string millis;
          sequence = new_sequence (Int64.of_string millis);
        }
  | [ millis; sequence ] ->
      Ok { millis = Int64.of_string millis; sequence = Int.of_string sequence }
  | [ "*" ] ->
...
let parse_xrange_id (id : string) (last_id : id_t option)
  ...
  | _ -> (
      match String.split ~on:'-' id with
      | [ millis; sequence ] ->
          { millis = Int64.of_string millis; sequence = Int.of_string sequence }
      | [ millis ] -> (
          match id_type with
          | FromId -> { millis = Int64.of_string millis; sequence = 0 }
          | ToId ->
              { millis = Int64.of_string millis; sequence = Int.max_value })
      | _ -> min_id)

let%test_unit "parse xrange full" =
  [%test_eq: id_t]
    (parse_xrange_id "15" None FromId)
    { millis = 15L; sequence = 0 }

let validate_entry_id (id : id_t) (reference_id : id_t) :
    (id_t, string) Result.t =
  if Int64.(id.millis = 0L) && id.sequence = 0 then
    Error "ERR The ID specified in XADD must be greater than 0-0"
  else if Int64.(id.millis > reference_id.millis) then Ok id
  else if Int64.(id.millis = reference_id.millis) then
